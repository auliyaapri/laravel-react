...
export default function Index({ mahasiswa, search: initialSearch }) {
  ...
  const changeName = (mahasiswa) => {
    return mahasiswa.data.map(mahasiswa => { // Menyesuaikan dengan struktur paginated
  ...
  return (
    <div style={{ margin: '2rem' }}>
      ...
      {flash.message && (
        <div className="alert mb-10 mt-3 text-2xl bg-amber-500 w-1/2 p-3 rounded-md text-white" id='flashSuccessAdd'>{flash.message}</div>
      )}
      ...
      <form action="" onSubmit={handleSearch} className='mb-4'>
        <input type="text" className='bg-slate-300 py-1 px-4 rounded-md' value={search} onChange={(e) => setSearch(e.target.value)} />

        <button type='submit' className='text-white bg-indigo-500 py-1 px-4 mb-5 rounded-md'>Search</button>
      </form>
        ...
        <tbody>
          {updatedMahasiswa.map((item, index) => (
            <tr key={index} style={{ border: '1px solid black' }}>
              <td style={{ border: '1px solid black' }} className='text-center'>{startNumber + 1}</td>
      ...
      <div className="pagination mt-4">
        {mahasiswa.links.map((link, index) => (
          <button
            ...
            className={`px-4 py-2 border rounded ${link.active ? 'bg-indigo-500 text-white' : 'bg-white'}`}
          >
